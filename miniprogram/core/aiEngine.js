...
class AIEngine {
    ...
    easyMove(board, player) {
        const opponent = player === types_1.Player.Black ? types_1.Player.White : types_1.Player.Black;
    ...
    findWinningMove(board, player) {
        ...
        for (let i = 0; i < board.length; i++) {
            for (let j = 0; j < board[i].length; j++) {
                ...
                board[i][j] = types_1.CellState.Empty;
                if (win) {
                    return { x: i, y: j };
                }
            }
    ...
    evaluatePosition(board, x, y, player) {
        const opponent = player === types_1.Player.Black ? types_1.Player.White : types_1.Player.Black;
        const playerValue = player === types_1.Player.Black ? types_1.CellState.Black : types_1.CellState.White;
        const opponentValue = opponent === types_1.Player.Black ? types_1.CellState.Black : types_1.CellState.White;
        // 模拟落子
        ...
        // 恢复棋盘
        board[x][y] = types_1.CellState.Empty;
    ...
    evaluateDirection(board, x, y, cellValue) {
        ...
        for (const dir of directions) {
            ...
            for (let i = 1; i < 6; i++) {
                ...
                else if (board[nx][ny] === types_1.CellState.Empty) {
                    break;
                }
                else {
                    blockCount++;
